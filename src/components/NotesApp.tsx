...
export default function NotesApp({ onClose: _onClose, onCornerDown, onNoteChange }: Props) {
  ...
  const pendingEditRef = useRef<{ id: string; oldWC: number; newWC: number } | null>(null)
    ...
    setNotes(prev => prev.map(n => {
      ...
      if (patch.content !== undefined && patch.content !== n.content) {
        ...
        if (!pendingEditRef.current || pendingEditRef.current.id !== id) {
          pendingEditRef.current = { id, oldWC: n.content.trim().split(/\s+/).filter(Boolean).length, newWC }
        } else {
          pendingEditRef.current.newWC = newWC
        }
        if (editDebounceRef.current) clearTimeout(editDebounceRef.current)
        editDebounceRef.current = setTimeout(() => {
          if (!pendingEditRef.current) return
          trackNoteEdited(pendingEditRef.current.oldWC, pendingEditRef.current.newWC)
